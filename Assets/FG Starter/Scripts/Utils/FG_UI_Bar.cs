...
namespace FG.Utils
{
    public class UI_Bar
    {
        ...
        {
            return FG_Utils.DrawSprite(outline.color, gameObject.transform, Vector2.zero, size + new Vector2(outline.size, outline.size), "Outline");
        }
        private void SetupBackground(Color backgroundColor)
        {
            background = FG_Utils.DrawSprite(backgroundColor, gameObject.transform, Vector2.zero, Vector2.zero, "Background");
            background.anchorMin = new Vector2(0, 0);
            background.anchorMax = new Vector2(1, 1);
        }
        private void SetupBar(Color barColor)
        {
            bar = FG_Utils.DrawSprite(barColor, gameObject.transform, Vector2.zero, Vector2.zero, "Bar");
