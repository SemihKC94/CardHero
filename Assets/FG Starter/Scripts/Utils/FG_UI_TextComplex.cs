...
namespace FG.Utils
{
    ...
    public class UI_TextComplex
    {
        ...
        {
            return FG_Utils.GetCanvasTransform();
        }
        ...
        public UI_TextComplex(Transform parent, Vector2 anchoredPosition, int fontSize, char iconChar, string text, Icon[] iconArr, Font font)
        {
            ...
            while (tmp.IndexOf(iconChar) != -1)
            {
                ...
                if (untilTmp.Trim() != "")
                {
                    Text uiText = FG_Utils.DrawTextUI(untilTmp, transform, new Vector2(textPosition, 0), fontSize, font);
                    textPosition += uiText.preferredWidth;
                }
                // Draw Icon
                int iconIndex = FG_Utils.Parse_Int(iconNumber, 0);
                Icon icon = iconArr[iconIndex];
                FG_Utils.DrawSprite(icon.sprite, transform, new Vector2(textPosition + icon.size.x / 2f, 0), icon.size);
                textPosition += icon.size.x;
            }
            if (tmp.Trim() != "")
            {
                FG_Utils.DrawTextUI(tmp, transform, new Vector2(textPosition, 0), fontSize, font);
            }
        }
