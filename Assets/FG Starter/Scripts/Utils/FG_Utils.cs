...
namespace FG.Utils
{
    public static class FG_Utils
    {
        ...
        public static FG_FunctionUpdater CreateWorldTextUpdater(Func<string> GetTextFunc, Vector3 localPosition, Transform parent = null)
        {
            TextMesh textMesh = CreateWorldText(GetTextFunc(), parent, localPosition);
            return FG_FunctionUpdater.Create(() => {
        ...
        public static void CreateWorldTextPopup(Transform parent, string text, Vector3 localPosition,Quaternion rot, int fontSize, Color color, Vector3 finalPopupPosition, float popupTime)
        {
            ...
            FG_FunctionUpdater.Create(delegate () {
        ...
        public static FG_FunctionUpdater CreateUITextUpdater(Func<string> GetTextFunc, Vector2 anchoredPosition)
        {
        ...
        public static FG_FunctionUpdater CreateMouseDraggingAction(Action<Vector3> onMouseDragging)
        {
            return CreateMouseDraggingAction(0, onMouseDragging);
        }

        public static FG_FunctionUpdater CreateMouseDraggingAction(int mouseButton, Action<Vector3> onMouseDragging)
        {
            bool dragging = false;
            return FG_FunctionUpdater.Create(() => {
                ...
                if (dragging)
                {
                    onMouseDragging(FG_Utils.GetMouseWorldPosition());
                }
                return false;
            });
        }

        public static FG_FunctionUpdater CreateMouseClickFromToAction(Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition)
        {
            return CreateMouseClickFromToAction(0, 1, onMouseClickFromTo, onWaitingForToPosition);
        }

        public static FG_FunctionUpdater CreateMouseClickFromToAction(int mouseButton, int cancelMouseButton, Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition)
        {
            int state = 0;
            ...
            return FG_FunctionUpdater.Create(() => {
                if (state == 1)
                {
                    if (onWaitingForToPosition != null) onWaitingForToPosition(from, FG_Utils.GetMouseWorldPosition());
                }
                ...
                if (Input.GetMouseButtonDown(mouseButton) && !FG_Utils.IsPointerOverUI())
                {
                    if (state == 0)
                    {
                        state = 1;
                        from = FG_Utils.GetMouseWorldPosition();
                    }
                    else
                    {
                        state = 0;
                        onMouseClickFromTo(from, FG_Utils.GetMouseWorldPosition());
                    }
                }
                return false;
            });
        ...
        public static FG_FunctionUpdater CreateMouseClickAction(Action<Vector3> onMouseClick)
        {
            return CreateMouseClickAction(0, onMouseClick);
        }

        public static FG_FunctionUpdater CreateMouseClickAction(int mouseButton, Action<Vector3> onMouseClick)
        {
            return FG_FunctionUpdater.Create(() => {
                if (Input.GetMouseButtonDown(mouseButton))
                {
        ...
        public static FG_FunctionUpdater CreateKeyCodeAction(KeyCode keyCode, Action onKeyDown)
        {
            return FG_FunctionUpdater.Create(() => {
