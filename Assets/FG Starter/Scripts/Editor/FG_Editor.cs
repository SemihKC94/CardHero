...
public class FG_Editor : EditorWindow
{
    //Main Toolbar
    ...
    {
        skin = (GUISkin)AssetDatabase.LoadAssetAtPath("Assets/FG Starter/Textures/EditorButtons/editorGUISkin.guiskin", typeof(GUISkin)); // Load Skin
        ...
        {
            FG_Editor window = (FG_Editor)EditorWindow.GetWindow(typeof(FG_Editor));
            window.Close();
        }
        GUILayout.EndArea();
        ...
        {
            GUILayout.Label("Please open FG/ Editor from top menu.", textError);
        }
    ...
    [MenuItem("Funjitsu/Editor/Scene Editor")]
    ...
    [MenuItem("Funjitsu/Editor/Editor Window")]
    public static void ShowWindow()
    {
        //SKC_Editor window = (SKC_Editor)EditorWindow.GetWindowWithRect<SKC_Editor>(new Rect(0,0,800,400));
        FG_Editor window = ScriptableObject.CreateInstance(typeof(FG_Editor)) as FG_Editor;
